...
Tests cover security-critical helpers (`_safe_claudex_path`, `_normalize_file_list`), session management, Pydantic model validation, and auto-session-ID generation. Test file uses PEP 723 inline metadata (same pattern as `server.py`).

## Architecture

**Single-file server** — all logic lives in `server/server.py` (~1950 lines). It's a FastMCP server (`FastMCP("codex")`) that exposes 9 tools:
...
**Slash commands** (`commands/*.md`): Define multi-step workflows for `/codex:plan`, `/codex:brainstorm`, `/codex:collab`, `/codex:status`. Each has YAML frontmatter with `allowed-tools`.
